...
video_path = os.path.join('videos', 'baseball1.mp4')
cap = cv2.VideoCapture(video_path)


output_path = 'output_video_slow.avi'
frame_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
frame_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
original_fps = int(cap.get(cv2.CAP_PROP_FPS))
slow_fps = max(1, int(original_fps * 0.25))  
out = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*'XVID'), slow_fps, (frame_width, frame_height))
